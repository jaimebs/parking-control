...
public class ParkingSpotController {
  ...
  public ResponseEntity<Page<ParkingSpotModel>> getAll(
    ...
    var parkingSpots = parkingSpotService.getAll(pageable);
    return ResponseEntity.status(HttpStatus.OK).body(parkingSpots);
  }

  @GetMapping("/{id}")
  public ResponseEntity<Object> getById(@PathVariable(value = "id") UUID id) {
    var parkingSpot = parkingSpotService.getById(id);
    if (!parkingSpot.isPresent()) {
      return ResponseEntity.status(HttpStatus.NOT_FOUND).body("Parking Spot not found!");
    }
    // Como o retorno é um Optional, para obter o objeto tem que ser acionado o get.
  ...
  public ResponseEntity<Object> save(@RequestBody @Valid ParkingSpotDto parkingSpotDto) {
    ...
    if (parkingSpotService.existsByApartmentAndBlock(parkingSpotDto.getApartment(), parkingSpotDto.getBlock())) {
      return ResponseEntity.status(HttpStatus.CONFLICT)
          .body("Conflict: Parking Spot already register for Apartment/Block");
    }
  ...
  public ResponseEntity<Object> update(@PathVariable(value = "id") UUID id,
    ...
    var parkingSpotObject = parkingSpotService.getById(id);
    if (!parkingSpotObject.isPresent()) {
      return ResponseEntity.status(HttpStatus.NOT_FOUND).body("Parking Spot not found!");
    }
    var parkingSpotModel = parkingSpotObject.get();
    ...
    var response = parkingSpotService.save(parkingSpotModel);
